export const meta = {
  name: "beads-drain",
  description: "Autonomous local Beads drain. Host-owned controller loop: Beads discovery, validation, mutation staging, and dry proof stay in the trusted adapter; implementation lanes are isolated worktrees.",
  harness: "drain",
  adapter: "beads",
  ...
  notes: "Machine invocation source of truth for beads-drain. Defaults to safe dry-run; autonomous-local mutates local Beads state only after verified gates and approval.",
  examples: [
...
if (args !== undefined && args !== null && (typeof args !== "object" || Array.isArray(args))) {
  throw new Error('beads-drain workflow args must be a JSON object when provided; omit args or pass an object such as {"mode":"dry-run"} or {"mode":"autonomous-local"}');
}
...
if (!["dry-run", "autonomous-local"].includes(mode)) {
  throw new Error('beads-drain mode must be "dry-run" or "autonomous-local"');
}
...
if (runtimeArgs.scope !== undefined && runtimeArgs.scope !== null
  ...
  throw 'beads-drain scope must be a JSON object when provided; a string/array would silently spread to a meaningless char-indexed scope and run an UNFILTERED drain — pass an object such as {"scope":{"priority":"P0"}} or omit it';
}
