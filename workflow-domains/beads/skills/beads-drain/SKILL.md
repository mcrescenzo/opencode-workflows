...
- Authority is approved once before launch. Non-dry autonomous runs must not depend on mid-run interactive permission prompts. Non-dry drain requires this one-time approval; the kernel verifies the server version floor (`GET /global/health`, minimum opencode 1.17.13) and asserts lane rooting/permissions deterministically at launch — there is no live-gate preflight step. A server below the minimum, or a failed rooting/permission assertion, refuses the launch instead of degrading silently.
...
Run the bundled workflow by name with `workflow_run({ name: "beads-drain", args: ... })`. Review the approval hash, effective authority profile, and dry-proof report. In autonomous-local the verified diff plan auto-applies; otherwise apply any diff plan through `workflow_apply`.
...
Active-runtime token, worktree, background, and notification side effects require the user's explicit non-dry launch approval described above — there is no separate live-gate release-check step. Normal `npm` tests and dry-run drains remain no-token by default.
